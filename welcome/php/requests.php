...
if ($title!="") {
	$code="SELECT * FROM requests WHERE (title LIKE '%".$title."%')";
	if ($date!="") {
	 	$code .="AND (date='".$date."')";
		if ($location!="") {
			$code .="AND (location LIKE '%".$location."%')";
			if ($type!="") {
				$code ="SELECT * FROM requests WHERE (title LIKE '%".$title."%') AND (location LIKE '%".$location."%') AND (date='".$date."') AND (type LIKE '%".$type."%') ORDER BY date DESC, time DESC";
			}else{
				$code ="SELECT * FROM requests WHERE (title LIKE '%".$title."%') AND (location LIKE '%".$location."%') AND (date='".$date."') ORDER BY date DESC, time DESC";
			}
		}else{
			if ($type!="") {
				$code .="AND (type LIKE '%".$type."%')";
			}else{
				$code="SELECT * FROM requests WHERE (title LIKE '%".$title."%') AND (date='".$date."') ORDER BY date DESC, time DESC";
			}
		}
	}else{
		if ($location!="") {
			...
			if ($type!="") {
				...
				$code ="SELECT * FROM requests WHERE (title LIKE '%".$title."%') AND (location LIKE '%".$location."%') ORDER BY date DESC, time DESC";
			}
		}else{
				$code ="SELECT * FROM requests WHERE (title LIKE '%".$title."%') ORDER BY date DESC, time DESC";
		}
	}
}else{
	if ($date!="") {
		 ...
		 if ($location!="") {
		 	...
		 	if ($type!="") {
				...
				$code ="SELECT * FROM requests WHERE (date='".$date."') AND (location LIKE '%".$location."%') ORDER BY date DESC, time DESC";
			}
		}else{
				$code ="SELECT * FROM requests WHERE (date='".$date."') ORDER BY date DESC, time DESC";
		}
	}else{
		if ($location!="") {
		 	...
		 	if ($type!="") {
				...
				$code ="SELECT * FROM requests WHERE (location LIKE '%".$location."%') ORDER BY date DESC, time DESC";
			}
		}else{
			if ($type!="") {
				$code ="SELECT * FROM requests WHERE (type LIKE '%".$type."%') ORDER BY date DESC, time DESC";
			}else{
				$code ="SELECT * FROM requests ORDER BY date DESC, time DESC";
			}
        ...
        <div class="info" id="data'.$i.'" style="display:none;">
	        ...
	        <div class="detail last">
	         	...
	         	<span class="detail_value">
		          	<form action="/play/welcome/php/participate.php" method="post">
